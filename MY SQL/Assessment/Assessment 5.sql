...
select C.customerId, C.FirstName, C.LastName, O.orderId, O.orderdate from Orders as O
join customers as C
on C.customerId=O.customerId; 
-- 5. Display the total number of orders placed by each customer.

select C.FirstName, count(O.orderId) from Orders as O
join customers as C
on C.customerId=O.customerId
...
select P.ProductName, O.OrderId, O.quantity from orders as O
join Products as P
...
select C.FirstName, O.orderID , C.country from Orders as O
join Customers as c
on C.customerId=O.customerId
where country= 'USA';
...
select C.firstname, C. state, O.orderId from Orders as O
join Customers as c
on C.customerId=O.customerId
...
select productId, productName, ReleaseDate from products
where releaseDate>'2022-01-01';

-- 16. Retrieve the orders with a specific payment status (e.g., 'Paid').

select orderId, PaymentStatus from orders
...
select productName, weight from products;
...
select ProductName, Releasedate from products;

-- 27. Retrieve the orders placed by customers with a specific zip code (e.g., '12345').

select C.FirstName, O.orderID , C.Zipcode from Orders as O
join Customers as c
on C.customerId=O.customerId
where Zipcode= '12345';
